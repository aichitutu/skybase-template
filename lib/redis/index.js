...
// 此常量用于保存单机锁的key
// 通常做法是让key作为字符串保存在redis中，但是考虑到，一般由某个进程加的锁，它自己也要负责解锁，所以可以把锁存到进程内存，这样解锁还快一些
const locks = {}
...
class HRedis {
  ...
  constructor (redis, option = {}) {
    ...
    this.prefix = option.prefix || packageJson.name
  ...
  get rd () {
    return this._redis || global.redis || ($.err('无redis连接，请检查redis是否已启动') && null) || null
  }
   ...
   * @param {boolean} [symbolKey=false] 返回的解锁key是字符串还是Symbol。如果是Symbol，则加锁和解锁必须在同一个线程内执行。
   * @return symbol|string|boolean 这里返回的是一个随机数，用于解锁时作为钥匙(key)
   * */
  async lock (lockName, msec, { symbolKey = false } = {}) {
    if (!msec || msec < 0) msec = 5000
    const kn = this.getKeyName(lockName, 'localLock')
    if (locks[kn]) { // 把内存也当作一道锁。这个可以不要，要的话能快一点，虽然过了单元测试，但是不知道会不会引发别的问题
      return false
    }
    let key = symbolKey ? Symbol(kn) : Date.now() + Math.random().toString()
    const res = await this.rd.set(kn, symbolKey ? 1 : key, 'NX', 'PX', msec) === 'OK'
    if (res) {
      if (symbolKey) {
        locks[kn] = key
   ...
   * @param {boolean} [symbolKey=true] 加锁时这个参数传了什么，就再传一次，不过即使加锁时传了该参数，这里也可以不传，只是传的话会少判断些
   * */
  async unLock (lockName, lockKey, { symbolKey = false } = {}) {
    const kn = this.getKeyName(lockName, 'localLock')
    if (locks[kn] !== lockKey && (symbolKey || lockKey !== (await this.rd.get(kn)))) {
      return false
    }
    const delRes = await this.rd.del(kn)
    if (delRes > 0) {
      if (locks[kn] === lockKey) { // await 到这可能会执行其他事件循环，所以要重新判断一遍才删得安心
        delete locks[kn]
      }
      return true
    }
  ...
  async set (key, value, expire) {
    ...
    if (expire) {
      return await this.rd.set(kn, value, 'EX', expire) === 'OK'
    } else {
      return await this.rd.set(kn, value) === 'OK'
    }
  }
  ...
  async hexists (key, field) {
    ...
    return this.rd.hexists(kn, field)
  }
  ...
  /**
   * 一口气把哈希集的所有field，field越多，取的速度越慢，慎用。但比上一个方法快
   * */
  async hkeys (key) {
    const kn = await this.getKeyName(key, HRedis.TYPES.HASH)
    return this.rd.hkeys(kn)
  }
  ...
  async zRangeByScore (key, min, max, withScores, offset, count) {
    ...
    if (typeof min === 'string') {
      if (['-', '-inf'].includes(min)) {
        ...
        $.err(`[zRangeByScore]min的值无法识别：${min}`)
        return []
      }
    }
    ...
    if (typeof max === 'string') {
      if (['+', '+inf'].includes(max)) {
        ...
        $.err(`[zRangeByScore]max的值无法识别：${max}`)
        return []
      }
    }
  ...
  async zRemRangeByScore (key, min, max) {
    ...
    if (typeof min === 'string') {
      if (['-', '-inf'].includes(min)) {
        ...
        $.err(`[zRemRangeByScore]min的值无法识别：${min}`)
        return 0
      }
    }
    ...
    if (typeof max === 'string') {
      if (['+', '+inf'].includes(max)) {
        ...
        $.err(`[zRemRangeByScore]max的值无法识别：${max}`)
        return 0
      }
    }
